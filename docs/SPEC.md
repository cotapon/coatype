...
6. [STT クライアント (OpenAI 互換)](#stt-クライアント-openai-互換)
...
macOS 向けメニューバーアプリ。ショートカットキーを押している間（Push-to-Talk）またはトグル操作でマイクから音声を録音し、OpenAI 互換の STT API でテキストに変換してアクティブなアプリケーションに直接入力する。

| 項目 | 値 |
|---|---|
| アプリ名 | CoAType (コエタイプ) |
| Bundle ID | `jp.co.cotapon.coatype` |
| プラットフォーム | macOS 13.0 (Ventura) 以降 |
| フレームワーク | Tauri v2 |
| フロントエンド | React 18 + Vite |
| バックエンド | Rust (stable) |
| STT エンドポイント | ユーザーが Settings のモデル設定で指定 (OpenAI 互換エンドポイント) |
| STT モデル | ユーザーが Settings のモデル設定で指定 (既定は空。例: `whisper-1`, `whisper-large-v3`) |
...
`language` 設定 (BCP-47 言語コード: `ja`, `en`, `zh` 等) を STT API の `language` パラメーターに渡す。
...
interface Settings {
  ...
  llm_correct: boolean;
  api_base: string;           // STT エンドポイントのベース URL (現行は stt.base_url に移行済み)
}
...
RMS < 300 の場合は STT API を呼ばずに空文字を返す。

> Whisper 系モデルは無音・環境音のみの音声に対して「ありがとうございます」「ありがとうございました」等を幻覚する。この閾値でフィルタリングすることで誤挿入を防ぐ。

---

## STT クライアント (OpenAI 互換)

OpenAI 互換の音声文字起こし API を呼ぶ汎用クライアント(`src-tauri/src/api/whisper.rs` の `WhisperClient`)。
`base_url` / `model` / `auth_kind` はすべてユーザー設定で、OpenAI・Groq・ローカル Whisper サーバー等どれでも利用できる。
...
| `model` | ユーザー設定のモデル名 (例: `whisper-1`, `whisper-large-v3`) |
...
パス: `~/Library/Application Support/jp.co.cotapon.coatype/settings.json`
...
パス: `~/Library/Application Support/jp.co.cotapon.coatype/dictionary.json`
...
パス: `~/Library/Application Support/jp.co.cotapon.coatype/history.db` (SQLite)
...
- Keychain service: `jp.co.cotapon.coatype` / account: `api-key`
