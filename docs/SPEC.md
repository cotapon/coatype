...
macOS 向けメニューバーアプリ。ショートカットキーを押している間（Push-to-Talk）またはトグル操作でマイクから音声を録音し、cotapon ML プラットフォームの Whisper API でテキストに変換してアクティブなアプリケーションに直接入力する。
...
| バックエンド | Rust (stable) |
| Whisper エンドポイント | (ユーザーが設定。環境変数 `COATYPE_API_BASE` または Settings の API Base URL にて指定) |
...
└─────────────────────────────────────────────────────────┘
         ...
         ▼
  cotapon ML Platform (Whisper API)
```
